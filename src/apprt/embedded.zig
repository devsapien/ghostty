...
pub const App = struct {
    /// On Linux, GTK requires drawing from the main/app thread.
...
pub const Platform = union(PlatformTag) {
    ...
    /// Linux embedded platform — the embedder passes a GtkGLArea pointer.
    ...
    pub fn init(tag_int: c_int, c_platform: C) !Platform {
        ...
        return switch (tag) {
            ...
            .linux => linux: {
                ...
                break :linux .{ .linux = .{ .gl_area = config.gl_area orelse return error.GLAreaMustBeSet } };
            },
        };
    }
};
...
pub const CAPI = struct {
    ...
    /// Notify Ghostty that the OpenGL display (GtkGLArea) has been realized and
    /// the GL context is now valid. Must be called from the main thread.
    export fn ghostty_surface_display_realized(surface: *Surface) void {
        surface.core_surface.renderer_thread.renderer.displayRealized() catch |err| {
            log.err("displayRealized error: {}", .{err});
        };
    }

    /// Notify Ghostty that the OpenGL display (GtkGLArea) has been unrealized
    /// and the GL context is no longer valid.
    export fn ghostty_surface_display_unrealized(surface: *Surface) void {
        surface.core_surface.renderer_thread.renderer.displayUnrealized();
    }

    /// Mark the surface dirty so it will be re-rendered on the next tick.
    export fn ghostty_surface_mark_dirty(surface: *Surface) void {
        surface.core_surface.renderer_thread.renderer.markDirty();
    }
